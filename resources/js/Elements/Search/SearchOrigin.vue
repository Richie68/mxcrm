<template>
  <div class="relative z-20" v-if="show">
    <div class="flex">
      <input
        type="search"
        name="search"
    ...
    >
      <li
        ...
        :key="result.uuid"
        @click="setOrigin(result)"
      >
        <div class="flex justify-between">
          <span>{{ result[label] }}</span> <span>{{ result.branch.name }}</span>
        </div>
      </li>
...
const emit = defineEmits(['set-origin-change', 'close-it'])
...
watch(search, debounce(function (value) {
  if(search.value == '') {
    ...
    axios
      .post(route('api.search-origins'), { search: value })
      .then( response => {
